...
# get data 
nihao, maggies = D.nihao(filters, dust=False) 
...
def log_like(tt, photo_obs, photo_ivar_obs, filters=None):
    ...
    '''
    _tt = np.concatenate([tt, [0., 0., -0.7]])
    # calculate SED model(theta) 
    _sed = m_nmf.sed(_tt, 0.022229, filters=filters)
    _, _flux, photo = _sed
...
if not keep: 
    ...
    p0 = np.load(os.path.join(dat_dir, 'mcmc/mcmc.nodust.%i.%i.npy' % (igal, iang)))[-1,:,:]
...
np.save(os.path.join(dat_dir, 'mcmc/mcmc.nodust.%i.%i.npy' % (igal, iang)),
